...
topology = 'fc'
mode = 'tpos'
w_t_ranger = np.arange(0.475, 0.775, 0.025)
...
################################################
step = 0.1
arange_accuracy = np.arange(0.5, 0.775, step)       
...
if mode == '':  # doesn't make any sense with any other mode
    ...
    for a,b in ((0.9,1.), (0.95,1.), (0.97,1.), (0.99,1.)):
        ...
        #  init
        init = np.load('./results/{}_weights_npy/{}_weights_acc-0.5-0.525.npy'.format(topology, init_prefix), allow_pickle=True)
        ...
        db.draw_bipartite_graph(init,
                                ...
                                title='Connectivity graph last layer 0.5-0.525-accuracy - Negative Weights',
                                showfig=True,
                                savefig=save1) 
        #  fin
        fin = np.load('./results/{}_weights_npy/{}_weights_acc-0.8-0.825.npy'.format(topology, fin_prefix), allow_pickle=True)
        ...
        db.draw_bipartite_graph(fin, 
                                actual_size=(init.shape[0],2),
                                title='Connectivity graph last layer 0.8-0.825-percentile - Negative Weights',
                                showfig=True,
                                savefig=save2)
        ...
        db.draw_bipartite_graph(init,
                                actual_size=(init.shape[0],init.shape[1]),
                                title='Connectivity graph last layer 0.5-0.525-accuracy - Positive Weights',
                                showfig=True,
                                savefig=save1) 
        #  fin
        fin = np.load('./results/{}_weights_npy/{}_weights_acc-0.8-0.825.npy'.format(topology, fin_prefix), allow_pickle=True)
        ...
        db.draw_bipartite_graph(fin, 
                                actual_size=(init.shape[0],2),
                                title='Connectivity graph last layer 0.8-0.825-percentile - Positive Weights',
                                showfig=True,
                                savefig=save2)
