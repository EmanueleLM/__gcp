...
init_weights = np.load('../adj_matrices/IN_0-0.npy', allow_pickle=True)
fin_weights = np.load('../adj_matrices/BN_0-last.npy', allow_pickle=True)

for i in range(1,35):
    tmp1 = np.load('../adj_matrices/IN_'+str(i)+'-0.npy', allow_pickle=True)
    tmp2 = np.load('../adj_matrices/BN_'+str(i)+'-last.npy', allow_pickle=True)
    for j in range(8):
...
for i in range(8):
    plt.title('[HISTOGRAM PARAMETERS PRE-POST TRAINING]Layer '+ weights_name[i])
