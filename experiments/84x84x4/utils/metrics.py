...
def nodes_strength(i_s, f_s, dst, show=True):
    ...
    for i in init_s.keys():
        ...
        plt.savefig(dst + 'hist_nodes_strengths_init_'+weights_name[o]+'.svg')
        plt.savefig(dst + 'hist_nodes_strengths_init_'+weights_name[o]+'.png')
        plt.pause(0.05)
...
def Yk(i_s, f_s, card_init, i_s_squared, f_s_squared, dst, show=True):
    """
    ...
    for key in card_init.item().keys():
        nodes_degrees[key] = card_init.item().get(key)
