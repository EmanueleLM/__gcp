## Tutto questo e' da fare su tutte le reti neurali ottime, mediando sui pesi di ciascun layer
...
final_weights = np.load('adj_gen_last.npy', allow_pickle=True)
...
kernels_shapes = [(8, 8, 3, 16), (4, 4, 16, 32)]
...
init_rec_field1 = np.zeros(shape=(210, 160, 3, 16))
fin_rec_field1 = np.zeros(shape=(210, 160, 3, 16))

for i in range(16):
	for n in range(0, 204, 4):
		for m in range(0, 156, 4):
...
# plot each single kernel
for i in range(16):
    plt.title('Initial kernel n.' + str(i))
    plt.imshow(initial_weights[0][:,:,:,i])  # first 3 channels as RGB
    plt.pause(0.05)
    plt.title('Final kernel fields, n.' + str(i))
    plt.imshow(final_weights[0][:,:,:,i])  # first 3 channels as RGB
    plt.pause(0.05)
    print("Distance between receptive fields pre and post learning: ", np.linalg.norm(init_rec_field1[:,:,:3,i]-fin_rec_field1[:,:,:3,i]))
plt.show()
